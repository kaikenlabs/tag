...
- `tag template lint` now reports a **warning** (`empty-generator`) for a directory under `_generators/` that holds no template files. Since #436 `tag generate` refuses to treat such a directory as a generator, so it silently disappears from `tag generate list` and from resolution — a template author previously only found out at generate time, in someone else's project. A directory holding only `tag.template.json`, or only subdirectories, is reported too: template files are not loaded recursively. The check reuses `engine.HasTemplateFiles`, the same predicate the loader uses, so lint and `tag generate` cannot drift. Severity is a warning rather than an error deliberately: a template can legitimately be mid-authoring, `tag template lint` gates CI for some users, and an error would fail templates that lint clean today. Only `_generators/` is scanned — the `.tag/` layout a template may also ship (#431) is not, which keeps this check's scope identical to the reserved-name check it shares a loop with (#439)
...
- **Potentially breaking**: `tag doctor` reports a warning, and so exits 1 instead of 0, for a project whose `.tag/<template>/_generators/` holds a directory with no template files — the condition #439 added to `tag template lint`. `doctorCheckTemplates` maps any lint warning to a doctor warning and `doctorAction` returns exit code 1 for warnings, so a project that was otherwise clean changes verdict without its source changing. This is deliberate: a generator that `tag generate` will not see is exactly what `tag doctor` exists to surface, and the alternative — filtering the warning out in doctor — would give doctor a second, quieter definition of what a broken template is. `tag template lint` itself is unaffected and still exits 0, because it gates on errors only. Projects using `tag doctor` in CI should re-run once after upgrading (#439)
...
- `tag scaffold <remote-ref> --dry-run` still wrote to `<cwd>/.tag/lock.json` — the write #432 fixed for the shared library was untouched here, so a preview run created or refreshed a lockfile entry the same as a real one. `verifyTemplateLock`/`lockfile.VerifyAndMaybeUpdate` now take a `DryRun` option: where a real run would create or refresh the entry (a template's first use, or any run with `--update-lock`), the dry run prints `(dry-run) would pin <ref> in .tag/lock.json` to stderr instead and writes nothing; an entry that already exists and still matches is silent, same as before. This line is not suppressed under `--format json` the way the #432 library announcement is, since it always goes straight to stderr rather than through a JSON-mode writer. Verification is unaffected: a checksum mismatch against an existing entry still fails the run with the same error a real run gives. Separately, a Cookiecutter template detected under `--dry-run` is now refused outright — `Cannot convert during --dry-run. Convert it first, then scaffold the converted template: tag convert cookiecutter <ref>` — instead of, on a TTY, prompting to convert and writing the fully converted template to disk; the refusal is checked before the pre-existing non-interactive guard, so `--dry-run --no-input` and `--dry-run --format json` also get the dry-run-specific message rather than the generic non-interactive one. Behaviour without `--dry-run` is unchanged (#442)
- `tag scaffold <remote-ref> --dry-run` wrote a real entry into the shared template library — `library.json` plus a full copy of the template under `templates/<name>/` — despite the flag's promise to write nothing. `addToLibrary` ran the same `tag lib add` path regardless of `DryRun`, so a preview run left the same on-disk footprint as a real one. Text mode now prints `(dry-run) would add template to library as "<name>"` in place of the write, or the pre-existing `Template "<name>" already in library` message when that name is already held by the same source — the same message a real run prints, since the guard sits after that check. Under `--format json` the announcement is not printed at all: `addToLibrary`'s writer is `io.Discard` in JSON mode, so stdout still carries exactly one JSON document, unchanged from before this fix — a `--dry-run --format json` document has no field indicating whether a real run would have written a library entry. This applies to both paths that reach the library add: a remote scaffold without `--no-library`, and a local template scaffolded with `--add-to-lib`. Behaviour without `--dry-run` is unchanged (#432)
- `tag generate <name>` decided "there is a generator" from directory existence alone, so an empty or template-less directory named `foo` silently beat a working same-named bundle `foo` — the run printed `Generated: 0 created, 0 skipped, 0 overwritten, 0 modified` and exited 0 having written nothing. A directory now only counts as a generator if it holds at least one top-level template file — any file besides the reserved `tag.template.json`, checked non-recursively, so a directory holding only subdirectories does not qualify. This is enforced everywhere a generator is resolved or listed: both library roots (`.tag/` and `_generators/`, including the fallback from an empty `.tag/foo` through to a populated `_generators/foo`), the project-local `.tag/` fallback, members of a self-contained bundle, `tag generate list`, `tag template list`, and shell completion. Three behaviour changes users may notice: (1) a same-named bundle now wins when the generator directory has no templates — precedence is unchanged when the generator directory genuinely has templates; (2) an empty or template-less directory no longer appears in `tag generate list` / `tag template list` / shell completion; (3) `tag generate foo` where `.tag/foo` is empty and no same-named bundle exists now fails loudly with "generator not found" (exit 3) instead of exiting 0 having done nothing — a self-contained bundle with an empty member directory now fails the same way instead of reporting success. A directory that exists but cannot be read (e.g. permissions) is unaffected by this fix: it is still treated as a generator and still fails loudly, rather than silently falling through to a bundle (#436)
