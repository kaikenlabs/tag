...
| D1/D2 | `output_dir` names the tree root | **Fixed by #390** — `project_root` is now emitted in `scaffoldDoc` and in the text summary's `Output:`/`cd` lines. Originally: **Fails — highest-value finding** | Wrapper template + explicit `--output` does **not** unwrap (`scaffold.go:281-290`): files land in `<output>/<project_name>/` while `output_dir` reports the parent. `ScaffoldResult.ProjectRoot` already held the right value (`types.go:61`) and was simply not emitted. Note `project_root` marks where the project begins, not the full extent of the output: a template with content beside the wrapper writes it under `output_dir`, so publishers must walk `files[]`. |
...
| 2 | ~~Emit `project_root` in `scaffoldDoc`~~ **Done (#390)** | **~1h** | No (additive) | Anyone scripting scaffold; fixes the stale text summary (`scaffold.go:588,595`) |
