...
- **Generator directories** — A directory under `_generators/` must hold at least one template file, or `tag generate` will not treat it as a generator and it will be absent from `tag generate list`. A directory holding only `tag.template.json`, or only subdirectories, is reported as a **warning** (template files are not loaded recursively). Only `_generators/` is checked; a `.tag/` directory a template may also ship is not.
