...
func TestUT_ScaffoldFromRef_LocalTemplateDir(t *testing.T) {
	...
	err := scaffoldFromRef(ctx, []string{templateDir, "test-project"}, formatText)
...
func TestUT_ScaffoldFromRef_LibraryTemplate_WithOutput(t *testing.T) {
	...
	err := scaffoldFromRef(ctx, []string{"my-go-api", "test-api"}, formatText)
	require.NoError(t, err)
...
func TestUT_ScaffoldFromRef_InvalidLocalPath(t *testing.T) {
	...
	err := scaffoldFromRef(ctx, []string{"./nonexistent-template-dir"}, formatText)
	require.Error(t, err)
